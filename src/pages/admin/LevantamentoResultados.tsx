...
export default function LevantamentoResultados() {
  ...
  const { data: respostas = [], isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      // Especificando o schema 'crm' conforme definido no types.ts
      ...
      {respostas.length === 0 ? (
        <Card className="p-12 text-center border-dashed">
          <div className="flex flex-col items-center gap-3">
            ...
            <Button variant="link" onClick={() => refetch()}>Tentar carregar novamente</Button>
          </div>
        </Card>
      ) : (
        <>
                    ...
                    <TableBody>
                      {filteredRespostas.map((r) => (
                        <TableRow key={r.id}>
                          ...
                          <TableCell className="max-w-[200px] truncate" title={r.ladrao_tempo || ""}>
                            {r.ladrao_tempo}
                          </TableCell>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {respostas.filter(r => r.maior_sonho).map((r) => (
                      <Card key={r.id} className="overflow-hidden group cursor-pointer border-none shadow-md" onClick={() => handleOpenDetails(r)}>
